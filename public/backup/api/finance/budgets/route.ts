...
export async function POST(req: Request) {
  try {
    ...
    const { name, category, amount, startDate, endDate, period, description, alertThreshold } = body;

    if (!name || !category || !amount || !startDate || !period) {
    ...
    const budget = await prisma.budget.create({
      data: {
        ...
        description,
        alertThreshold: alertThreshold || 0.8, // Default to 80% if not provided
        userId: session.user.id
      }
    });
